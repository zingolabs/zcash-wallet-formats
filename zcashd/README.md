...
| **bestblock**        | The current best block of the blockchain.                      | -                                                                                                                              | `CBlockLocator`                                                     | `vector<uint256>` (vector of block hashes) (WIP: how are they stored?)                                 |
| **chdseed**          | Encrypted HD seed.                                             | `uint256` (seed fingerprint)                                                                                                   | `vector<unsigned char>`                                             | `vector<unsigned char>` (WIP: how is it encrypted & stored)                                            |
| ckey\*               | Encrypted transparent pubkey and private key.                  | `vector<unsigned char>` = `byte` (public key length) + `unsigned char[33 \| 65]`(public key in compressed/uncompressed format) | `vector<unsigned char>`                                             | `vector<unsigned char>` (WIP: how is it encrypted & stored)                                            |
| csapzkey\*           | Encrypted Sapling pubkey and private key.                      | `libzcash::SaplingIncomingViewingKey` = 32-byte ivk in little-endian, padded with zeros in the most significant bits           | `libzcash::SaplingExtendedFullViewingKey` + `vector<unsigned char>` | `extfvk` (Extended Full Viewing Key) + `vchCryptedSecret` (WIP: how is it encrypted & stored)          |
| **cscript**          | Serialized script, used inside transaction inputs and outputs. | `uint160`                                                                                                                      | `CScript`                                                           |                                                                                                        |
| czkey\*              | Encrypted Sprout pubkey and private key.                       | `libzcash::SproutPaymentAddress` = `uint256` (spending key) + `uint256` (public key)                                           | `uint256` + `vector<unsigned char>`                                 |                                                                                                        |
| **defaultkey**       | Default Transparent key.                                       | -                                                                                                                              | `CPubKey`                                                           | `byte` (public key length) + `unsigned char[33 \| 65]`(public key in compressed/uncompressed format)   |
...
| key\*                | Transparent pubkey and privkey.                                | `CPubKey` = `byte` (public key length) + `unsigned char[33 \| 65]` (public key in compressed/uncompressed format)              | `CPrivKey`                                                          | `private_key` + SHA256(`public_key`+`private_key`)                                                     |
| keymeta\*            | Transparent key metadata.                                      | `CPubKey` = `byte` (public key length) + `unsigned char[33 \| 65]` (public key in compressed/uncompressed format)              | `CKeyMetadata`                                                      | `string` (hdKeypath, optional HD/zip32 keypath) + `uint256` (seed fingerprint)                         |
