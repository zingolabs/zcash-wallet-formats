...
| <span id="CPrivKey">`CPrivKey` (WIP)</span>                                                         | Serialized private key, with all parameters included.               | `unsigned char[279]` (private key, serialized as a DER ECPrivateKey type from section C.4 of SEC 1) [^3]                                                                                                                   |
| <span id="CKeyMetadata">`CKeyMetadata`</span> (WIP)                                                 |                                                                     | `string` (hdKeypath, optional HD/zip32 keypath) + `uint256` (seed fingerprint)                                                                                                                                             |
| <span id="CMasterKey">`CMasterKey`</span> (WIP)                                                     | Master key for wallet encryption. Encrypted using AES-256-CBC. [^4] | `vector<unsigned char>` (vchCryptedKey) + `unsigned char[8]` (salt) + `unsigned int` (0 = EVP_sha512 [^5] \| 1 = scrypt [^6]) + `unsigned int` (derive iterations) + `vector<unsigned char> (extra parameters)`            |
