...
Each `dat` file is a BerkeleyDB store[^1]. Entries are stored as follows:
...
| <span id="CMasterKey">`CAccountingEntry`</span> (WIP: LIMITED_STRING)                               |                                                       | `int64_t` (credit_debit) + `int64_t` (unix timestamp) + `string` (other_account)                                                                                                                                           |
